...
import { ArrowRight, ArrowLeft, Clock, MapPin, Sparkles } from 'lucide-react';
...
export function JournalIndexClient({ initialArticles }: Props) {
  const { t, locale, isRTL } = useLanguage();
  const jt = t.journalPage;

  const localizedArticles = initialArticles.map((article) =>
    getLocalizedArticle(article, locale)
  );
  const [featuredStory, ...otherStories] = localizedArticles;
    ...
    >
      ...
      <section className="pt-12 sm:pt-16 pb-12 px-4 sm:px-8 border-b border-[#0F2E23]/10">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="max-w-3xl pt-1 pl-4 sm:pl-6 rtl:pl-0 rtl:pr-4 sm:rtl:pr-6 border-l-2 rtl:border-l-0 rtl:border-r-2 border-[#2E6B57]/50 space-y-2">
      ...
      {/* ── Main Editorial Feed (Alternating Editorial Alignment) ── */}
                  ...
                  <div className="lg:col-span-7 relative aspect-[16/10] w-full rounded-2xl sm:rounded-3xl overflow-hidden shadow-md group-hover:shadow-2xl transition-all duration-500 bg-black/5">
                    ...
                    <div className="absolute top-4 left-4 rtl:left-auto rtl:right-4 z-10">
                      <span className="px-3.5 py-1 rounded-full text-xs font-mono font-medium uppercase tracking-wider bg-black/65 text-white backdrop-blur-md border border-white/20">
                        {jt.featuredDispatch} • {featuredStory.category}
                      </span>
                    </div>

                    <div className="absolute bottom-4 left-4 rtl:left-auto rtl:right-4 text-white text-xs font-mono flex items-center gap-2 opacity-90">
                  ...
                  <div className="lg:col-span-5 space-y-5">
                    ...
                    <div className="pt-4 flex items-center justify-between border-t border-[#0F2E23]/8">
                      <span className="text-xs font-mono text-[#0F2E23]/50">
                        {jt.by} {featuredStory.author.name}
                      </span>
                      ...
                      <span className="inline-flex items-center gap-2 text-xs font-sans font-semibold uppercase tracking-wider text-[#2E6B57] group-hover:text-[#0F2E23] transition-colors">
                        <span>{jt.readFullDispatch}</span>
                        {isRTL ? (
                          <ArrowLeft className="w-3.5 h-3.5 group-hover:-translate-x-1.5 transition-transform text-[#39C27D]" />
                        ) : (
                  ...
                  >
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14 items-center">
                      ...
                      >
                        ...
                        <div className="absolute bottom-3.5 left-4 rtl:left-auto rtl:right-4 text-white text-xs font-mono flex items-center gap-1.5 opacity-90">
                          <Clock className="w-3.5 h-3.5 text-[#39C27D]" />
                      ...
                      >
                        ...
                        <div className="pt-4 flex items-center justify-between border-t border-[#0F2E23]/8">
                          <span className="text-xs font-mono text-[#0F2E23]/50">
                            {jt.by} {story.author.name} • {story.date}
                          </span>

                          <span className="inline-flex items-center gap-2 text-xs font-sans font-semibold uppercase tracking-wider text-[#2E6B57] group-hover:text-[#0F2E23] transition-colors">
                            <span>{jt.readStory}</span>
                            {isRTL ? (
                              <ArrowLeft className="w-3.5 h-3.5 group-hover:-translate-x-1.5 transition-transform text-[#39C27D]" />
                            ) : (
                              <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1.5 transition-transform text-[#39C27D]" />
